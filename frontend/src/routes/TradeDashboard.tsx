    ...
    <Link to={`/trade/${proposal.market_state_id}`} className="block">
      <div className="bg-gray-800 rounded-lg shadow-sm border border-gray-700 p-5 hover:shadow-md transition-shadow">
        <div className="flex justify-between items-start mb-4">
          <Tooltip 
            content={
              <p className="text-sm text-gray-100 max-w-xl" style={{
          ...
          >
            <h3 className="text-lg font-semibold text-gray-100 line-clamp-2 leading-tight h-10 mr-1 overflow-hidden max-w-[180px]" style={{ height: '2.7rem' }}>
            {proposal.title}
            </h3>
          </Tooltip>
          <span className={`px-3 py-1 text-sm rounded-full whitespace-nowrap ${stateColor}`}>
            {stateLabel}
          </span>
        </div>
        ...
        <div className="flex flex-col">
          <div className="flex items-center">
            ...
            <span className="truncate font-medium text-gray-200">{proposal.dao_name}</span>
            {proposal.dao_verified && <VerifiedIcon className="ml-1 flex-shrink-0"/>}
          </div>
        </div>
      </div>
    </Link>
...
export function TradeDashboard() {
  const [selectedStates, setSelectedStates] = useState<string[]>(stateOptions.map(option => option.value));
  ...
  const filteredProposals = proposalsData?.data?.filter((proposal: ApiProposal) => 
