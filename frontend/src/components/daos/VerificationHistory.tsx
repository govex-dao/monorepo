...
export function VerificationHistory({ daoId, daoName, isSelected }: VerificationHistoryProps) {
  const { data: requests, isLoading } = useQuery<VerificationRequest[]>({
    queryKey: [QueryKey.VerificationHistory, daoId],
    queryFn: async () => {
      const response = await fetch(`${CONSTANTS.apiEndpoint}dao/${daoId}/verification-requests`);
      if (!response.ok) {
  ...
  if (isLoading) {
    return <div className="text-sm text-gray-500">Loading verification history...</div>;
  }

  if (!requests?.length) {
    return <div className="text-sm text-gray-500">No existing verification requests found</div>;
  }
