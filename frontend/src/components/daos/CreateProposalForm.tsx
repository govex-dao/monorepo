...
import { createProposalTransaction, CreateProposalData } from './proposal-transaction';
...
const tooltips = {
  ...
  outcomeMessages: "The first option must be 'Reject'. If there are two outcomes in total the second must be 'Accept'.",
...
const OutcomeMessages: React.FC<OutcomeMessagesProps> = ({ 
  ...
  const getMinAmounts = (daoData: DaoData | null) => {
      ...
      const minAssetAmount = parseFloat(convertToDisplayAmount(daoData.minAssetAmount, daoData.asset_decimals)) || 0;
      const minStableAmount = parseFloat(convertToDisplayAmount(daoData.minStableAmount, daoData.stable_decimals)) || 0;
  ...
  const removeOutcome = (index: number) => {
    ...
    if (outcomes.length === 3) {
      ...
      newCustomAmounts = [
          minAssetAmount, minStableAmount,
          minAssetAmount, minStableAmount
        ];
    } else {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center space-x-2">
        <label className="block text-sm font-medium text-gray-200">Outcome Messages</label>
...
const CreateProposalForm = ({walletAddress, daoIdFromUrl}: CreateProposalFormProps) => {
  ...
  const { data: daoData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`${CONSTANTS.apiEndpoint}daos?dao_id=${encodeURIComponent(daoIdFromUrl)}`);
      if (!response.ok) {
  ...
  useEffect(() => {
    setFormData(prev => ({
      ...
      minAssetAmount: daoData ? convertToDisplayAmount(daoData.minAssetAmount, daoData.asset_decimals) : prev.minAssetAmount,
      minStableAmount: daoData ? convertToDisplayAmount(daoData.minStableAmount, daoData.stable_decimals) : prev.minStableAmount
    }));
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name === 'outcomeMessages' ? value.split(',').map(msg => msg.trim()) : value,
  ...
  const handleDaoSelect = (daoData: DaoData | null) => {
    if (daoData) {
      setFormData(prev => ({
        ...
        minAssetAmount: convertToDisplayAmount(daoData.minAssetAmount, daoData.asset_decimals),
        minStableAmount: convertToDisplayAmount(daoData.minStableAmount, daoData.stable_decimals),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (titleLength > TITLE_MAX_LENGTH) {
            throw new Error(`Title exceeds maximum length of ${TITLE_MAX_LENGTH} characters (current: ${titleLength} characters)`);
        }
        if (metadataLength > METADATA_MAX_LENGTH) {
            throw new Error(`Metadata exceeds maximum length of ${METADATA_MAX_LENGTH} characters (current: ${metadataLength} characters)`);
        }
        if (descriptionLength > DETAILS_MAX_LENGTH) {
            throw new Error(`Description exceeds maximum length of ${DETAILS_MAX_LENGTH} characters (current: ${descriptionLength} characters)`);
        }
        ...
        if (showAdvancedSettings && customAmounts.length > 0) {
            ...
            for (let i = 0; i < outcomeCount; i++) {
                ...
                chainAmounts.push(convertToChainAmount(parseFloat(formData.minAssetAmount), daoData?.asset_decimals || 0));
                chainAmounts.push(convertToChainAmount(parseFloat(formData.minStableAmount), daoData?.stable_decimals || 0));
            }
        }
        ...
        {formData.daoObjectId && (
          <div className="bg-gray-900 rounded-lg p-3">
            ...
            <div>
              ...
              <p className="text-xs truncate">{truncateAddress(formData.assetType)}</p>
            </div>
            <div>
              <p className="text-sm font-medium">Stable Type:</p>
              <p className="text-xs truncate">{truncateAddress(formData.stableType)}</p>
            </div>
            <div>
                ...
                <div key={index} className="grid grid-cols-3 gap-4 items-center">
                  ...
                  <div>
                    <input
                      ...
                      min={parseFloat(convertToDisplayAmount(daoData?.minAssetAmount || '0', daoData?.asset_decimals || 0))}
                      className={`w-full p-2 rounded bg-gray-900 border ${
                        customAmounts[index * 2] < parseFloat(convertToDisplayAmount(daoData?.minAssetAmount || '0', daoData?.asset_decimals || 0))
                          ? 'border-red-500'
                          : 'border-gray-700'
                      }`}
                    />
                    {customAmounts[index * 2] < parseFloat(convertToDisplayAmount(daoData?.minAssetAmount || '0', daoData?.asset_decimals || 0)) && (
                      <p className="text-xs text-red-500 mt-1">
                        Below minimum ({convertToDisplayAmount(daoData?.minAssetAmount || '0', daoData?.asset_decimals || 0)})
                      </p>
                    )}
                  </div>
                  <div>
                    <input
                      ...
                      min={parseFloat(convertToDisplayAmount(daoData?.minStableAmount || '0', daoData?.stable_decimals || 0))}
                      className={`w-full p-2 rounded bg-gray-900 border ${
                        customAmounts[index * 2 + 1] < parseFloat(convertToDisplayAmount(daoData?.minStableAmount || '0', daoData?.stable_decimals || 0))
                          ? 'border-red-500'
                          : 'border-gray-700'
                      }`}
                    />
                    {customAmounts[index * 2 + 1] < parseFloat(convertToDisplayAmount(daoData?.minStableAmount || '0', daoData?.stable_decimals || 0)) && (
                      <p className="text-xs text-red-500 mt-1">
                        Below minimum ({convertToDisplayAmount(daoData?.minStableAmount || '0', daoData?.stable_decimals || 0)})
                      </p>
                    )}
                  </div>
                </div>
        ...
        {(daoData?.review_period_ms || daoData?.trading_period_ms) && (
          <p className="text-gray-200 text-sm mt-4">
            {(() => {
              ...
              return `Please be aware you may not be able to withdraw your liquidity for up to ${totalDays} ` + 
