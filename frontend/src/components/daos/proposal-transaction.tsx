...
export async function createProposalTransaction(
  ...
  try {
    ...
    async function prepareCoin(coins: { data: { coinObjectId: string, balance: string }[] }, minAmount: string, coinType: string) {
      ...
      const [splitCoin] = txb.splitCoins(primaryCoin, [txb.pure.u64(minAmount)]);
      return splitCoin;
    }

    // Create proposal using string arguments directly
    // First calculate max values for asset and stable amounts
    const getMaxAmounts = (amounts: number[]): { maxAsset: number, maxStable: number } => {
      let maxAsset = 0;
      let maxStable = 0;
    ...
    const { maxAsset, maxStable } = initialAmounts ? getMaxAmounts(initialAmounts) : { maxAsset: 10000, maxStable: 10000 };
    ...
    const [preparedAssetCoin, preparedStableCoin] = await Promise.all([
      ...
      prepareCoin(stableCoins, maxStable.toString(), `0x${formData.stableType}`)
    ]);
    ...
    txb.moveCall({
      ...
      arguments: [
        ...
        initialAmounts ? txb.pure.option('vector<u64>', initialAmounts) : txb.pure.option('vector<u64>', null),
        txb.object('0x6')
      ]
    });
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to create proposal transaction';
