...
const CreateDaoForm = () => {
  ...
  const [stableMetadata, setStableMetadata] = useState<CoinMetadata | null>(null);
  ...
  const getChainAmount = (amount: string, decimals: number): bigint => {
    try {
      ...
      return BigInt(Math.floor(parseFloat(cleanAmount) * Math.pow(10, decimals)));
    } catch (error) {
      ...
      const checkImage = async () => {
        try {
          ...
          if (contentLength && parseInt(contentLength) > 10 * 1024 * 1024) {
            setImageError(true);
            toast.error("Image size exceeds 10MB limit. Please use a smaller image.");
            return;
          }
  ...
  const tooltips = {
    assetType: "The asset that represents ownership over the DAO. Format: package::module::type",
    stableType: "The stable coin to price the asset in. Format: package::module::type",
    minAssetAmount: "The minimum amount of the asset the proposer must supply to create a proposal",
    minStableAmount: "The minimum amount of the stable coin the proposer must supply to create a proposal",
    daoName: "The name of your DAO",
    imageUrl: "If not set this will default to the URL for your asset coin. If your asset coin does not have a URL, set one here.",
    reviewPeriodMs: "The period of time in milliseconds between when a proposal is created and trading will begin.",
    tradingPeriodMs: "The trading period in milliseconds",
    assetMetadata: "The metadata object ID for the asset coin type selected above",
    stableMetadata: "The metadata object ID for the stable coin type selected above",
    ...
    twapThreshold: "% difference by which an outcome must be greater than Reject to pass"
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!assetMetadata?.id || !stableMetadata?.id) {
      setError('Valid coin metadata is required for both asset and stable coins');
      return;
    }
    if (!validateTypeFormat(formData.stableType)) {
      setError('Invalid Stable Type format. Expected format: package::module::type');
      return;
    }
    ...
    if (isNaN(parseFloat(formData.minAssetAmount)) || isNaN(parseFloat(formData.minStableAmount))) {
    ...
    try {
      chainAssetAmount = getChainAmount(formData.minAssetAmount, assetMetadata.decimals);
      chainStableAmount = getChainAmount(formData.minStableAmount, stableMetadata.decimals);
    ...
    try {
      ...
      const chainAdjustedTwapStepMax = Math.max(1, Math.round(formData.twapStepMax * 100));
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <label className="block text-sm font-medium">Min Asset Amount</label>
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <label className="block text-sm font-medium">Min Stable Amount</label>
              <div className="relative group">
                <InfoCircledIcon className="w-4 h-4 text-gray-400 hover:text-gray-600 cursor-help" />
                <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 w-64 z-50">
          ...
          <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <label className="block text-sm font-medium">Pre-trading Period (ms)</label>
            <div className="relative group">
              <InfoCircledIcon className="w-4 h-4 text-gray-400 hover:text-gray-600 cursor-help" />
              <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 w-64 z-50">
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <label className="block text-sm font-medium">Trading Period (ms)</label>
            <div className="relative group">
              <InfoCircledIcon className="w-4 h-4 text-gray-400 hover:text-gray-600 cursor-help" />
              <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 w-64 z-50">
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <label className="block text-sm font-medium">TWAP Start Delay (ms)</label>
            <div className="relative group">
              <InfoCircledIcon className="w-4 h-4 text-gray-400 hover:text-gray-600 cursor-help" />
              <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 w-64 z-50">
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <label className="block text-sm font-medium">TWAP Threshold (%)</label>
            <div className="relative group">
              <InfoCircledIcon className="w-4 h-4 text-gray-400 hover:text-gray-600 cursor-help" />
              <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 w-64 z-50">
        ...
        <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <label className="block text-sm font-medium">Overwrite default DAO image with new URL</label>
              <div className="relative group">
                <InfoCircledIcon className="w-4 h-4 text-gray-400 hover:text-gray-600 cursor-help" />
                <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 w-64 z-50">
                  {tooltips.imageUrl}
        ...
        </div> {/* End of advanced configuration section */}
        ...
        {/* Image preview section */}
          ...
          {imageError && (
            <p className="text-sm text-red-500">
              {formData.imageUrl ? "Error: Image is over 10MB or invalid URL" : "Failed to load image"}
            </p>
          )}
          </div>
