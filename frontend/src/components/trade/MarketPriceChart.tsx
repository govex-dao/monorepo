...
import { createChart, ISeriesApi, Time, ColorType, PriceScaleMode  } from 'lightweight-charts';
...
const MarketPriceChart = ({
  ...
  const handleRangeSelect = (range: string) => {
    ...
    const endTime = currentState === 1 
      ? Math.floor(Date.now() / 1000)
      : Math.floor(new Date(Number(tradingEnd || Date.now())).getTime() / 1000);
  ...
  // Fetch state changes
  const { data: stateChanges, error: stateError } = useQuery<StateHistoryResponse>({
    queryKey: ['stateHistory', proposalId],
    queryFn: async () => {
      const response = await fetch(`${CONSTANTS.apiEndpoint}proposals/${proposalId}/state-history`);
  ...
  const getInitialPrices = () => {
    ...
    if (initial_outcome_amounts && initial_outcome_amounts.length >= numOutcomes * 2) {
      const prices = Array.from({ length: numOutcomes }, (_, i) => {
        ...
        const fallbackPrice = BigInt(assetValue) === 0n ? 0 : Number(BigInt(stableValue)) / Number(BigInt(assetValue));
    ...
    const defaultPrice = BigInt(assetValue) === 0n ? 0 : Number(BigInt(stableValue)) / Number(BigInt(assetValue));
  ...
  const initialPrices = getInitialPrices();
    ...
    const chartData = React.useMemo(() => {
      ...
      [...swapEvents]
        ...
        .forEach(event => {
          const time = Math.floor(new Date(Number(event.timestamp)).getTime() / 1000);
          const priceValue = Number(BigInt(event.price)) /  (Number(BigInt(basisPoints)) * 1000000000000); //10^18
      ...
      const startTime = Math.floor(new Date(Number(tradingStart)).getTime() / 1000);
      const endTime = currentState === 1 
        ? Math.floor(Date.now() / 1000)  // Use current time if still trading
        : tradingEnd 
          ? Math.floor(new Date(Number(tradingEnd)).getTime() / 1000)
          : Math.floor(new Date(Number(swapEvents[swapEvents.length - 1]?.timestamp || tradingStart)).getTime() / 1000);
      ...
      return sampledData;
    }, [swapEvents, initialPrices, tradingStart, tradingEnd, basisPoints, outcome_count, currentState]);
  ...
  }, [chartData, outcome_count, outcome_messages, colors, selectedRange, currentState, tradingEnd]);
  ...
  const statusMessage = React.useMemo(() => {
    ...
    if (currentState === 2 || tradingEnd && winning_outcome != null)
        return winning_outcome ? `Trading period finished, winning outcome is: ${outcome_messages[Number(winning_outcome)]}` : 'Trading period finished';
    if (currentState === 1 && chartData.length <= 1) return 'No trading activity yet';
    return '';
  }, [currentState, tradingStart, tradingEnd, chartData, swapError, stateError]);
  return (
