...
const TradeForm: React.FC<TradeFormProps> = ({
  ...
  const updateFromAmount = (newAmount: string) => {
      ...
      const computedMin = Math.floor(expectedOut * (1 - TOLERANCE) * scaleFactor) / scaleFactor;
  ...
  const filteredTokens = useMemo(() => {
    ...
    useEffect(() => {
      ...
      }, [selectedOutcome, swapEvents, initial_outcome_amounts, asset_value, stable_value]);
  ...
  const handleTradeWithExisting = async (
    ...
    const totalBalance = validTokens.reduce((sum, token) => sum + BigInt(token.balance), 0n);
    ...
    if (validTokens.length > 1) {
        ...
        txb.moveCall({
          ...
          arguments: [
            ...
            txb.makeMoveVec({ elements: tokensToMerge.map(t => txb.object(t.id)) }),
            txb.object('0x6')
          ],
        });
  ...
  const handleTradeWithNewDeposit = async (txb: Transaction, amount: bigint) => {
    ...
    if (sortedCoins.length === 0) {
      throw new Error(`No ${tradeDirection === 'assetToStable' ? 'asset' : 'stable'} coins available in wallet`);
    }
    ...
    if (BigInt(sortedCoins[0].balance) >= amountBig) {
      ...
      if (totalBalance < amountBig) {
        throw new Error(`Insufficient ${tradeDirection === 'assetToStable' ? 'asset' : 'stable'} balance in wallet`);
      }
  ...
  const handleTrade = async () => {
    ...
    try {
      ...
      const amountScaled = tradeDirection === 'assetToStable' 
        ? BigInt(Math.floor(parseFloat(amount) * Number(assetScale)))
        : BigInt(Math.floor(parseFloat(amount) * Number(stableScale)));
      
      const expectedAmountOutScaled = tradeDirection === 'assetToStable'
        ? BigInt(Math.floor(parseFloat(expectedAmountOut) * Number(stableScale)))
        : BigInt(Math.floor(parseFloat(expectedAmountOut) * Number(assetScale)));
      
      console.log('trade-amounts (scaled)', amountScaled.toString(), expectedAmountOutScaled.toString());
      const txb = new Transaction();
      txb.setGasBudget(1000000000);

       const existingConditionalType = tradeDirection === 'assetToStable' ? 0 : 1;
       const hasExistingTokens = filteredTokens.some(t => 
         t.outcome === parseInt(selectedOutcome) && 
       ...
       if (hasExistingTokens) {
         const { tokenToSwap, totalBalance } = await handleTradeWithExisting(
          ...
          if (totalBalance >= amountScaled) {
            ...
            const splitCoin = await handleTradeWithNewDeposit(txb, amountScaled - totalBalance);
        ...
        const swapTarget = tradeDirection === 'assetToStable'
