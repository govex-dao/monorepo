...
import React, { Dispatch, SetStateAction, useMemo, useState } from 'react';
  ...
  const getOutcomeClass = (index: number) => {
    ...
    const colorClasses = ["text-red-300", "text-green-300", "text-blue-300", "text-yellow-300", "text-purple-300"];
...
const TradeInsight: React.FC<TradeInsightProps> = ({
  ...
  const truncatedMessage = useMemo(() => truncateMessage(outcomeMessage), [outcomeMessage])
      ...
      <div>
        ...
        <p className="leading-relaxed mt-2.5 text-xs">
          You expect the price to {isBuy 
            ...
            : <strong className="text-red-300">fall</strong>} if {" "}
        ...
        <p className="leading-relaxed mt-1 text-xs">
          ...
          the price of {isBuy ? stableSymbol : assetSymbol} at <span className="font-mono bg-black/30 px-2 py-0.5 rounded-md font-medium">${finalPrice?.toFixed(3)}</span> is too {isBuy ? 'low' : 'high'}.
        </p>
      </div>
    );
  }
