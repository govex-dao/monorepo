...
const TradeDirectionToggle: React.FC<TradeDirectionToggleProps> = ({ isBuy, setIsBuy }) => {
  return (
    <div className="flex bg-gray-800/70 p-1 rounded-lg relative">
      <div
        className={`absolute top-1 bottom-1 w-1/2 rounded-md transition-all duration-300 shadow-md ${isBuy ? 'left-1 bg-green-500' : 'left-[calc(50%-1px)] bg-red-500'
          }`}
      ></div>
      <button
        type="button"
...
export const TradeDirectionSwapButton: React.FC<TradeDirectionToggleProps> = ({ isBuy, setIsBuy }) => {
  return (
