...
fun test_get_asset_and_stable_supply_after_registration() {
    ...
    let asset_supply = coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0);
    let stable_supply = coin_escrow::get_stable_supply<TEST_COIN_A, TEST_COIN_B, COND_0_STABLE>(&escrow, 0);
...
fun test_mint_conditional_stable_basic() {
    ...
    // Verify supply updated
    let supply = coin_escrow::get_stable_supply<TEST_COIN_A, TEST_COIN_B, COND_0_STABLE>(&escrow, 0);
    assert!(supply == amount, 1);
...
fun test_mint_multiple_times_accumulates_supply() {
    ...
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 500, 0);

    // Mint second batch
    let coin2 = coin_escrow::mint_conditional_asset<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, 300, ctx
    );
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 800, 1);

    // Mint third batch
    let coin3 = coin_escrow::mint_conditional_asset<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, 200, ctx
    );
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 1000, 2);
...
fun test_mint_different_outcomes_independently() {
    ...
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 1000, 0);
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_1_ASSET>(&escrow, 1) == 2000, 1);
...
fun test_mint_conditional_asset_and_stable_for_same_outcome() {
    ...
    let asset_coin = coin_escrow::mint_conditional_asset<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, 1500, ctx
    );
    let stable_coin = coin_escrow::mint_conditional_stable<TEST_COIN_A, TEST_COIN_B, COND_0_STABLE>(
        &mut escrow, 0, 2500, ctx
    );

    // Verify independent supplies
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 1500, 0);
    assert!(coin_escrow::get_stable_supply<TEST_COIN_A, TEST_COIN_B, COND_0_STABLE>(&escrow, 0) == 2500, 1);
...
fun test_mint_zero_amount() {
    ...
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 0, 1);
...
fun test_mint_large_amount() {
    ...
    let coin = coin_escrow::mint_conditional_asset<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, large_amount, ctx
    );

    assert!(coin.value() == large_amount, 0);
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == large_amount, 1);
...
fun test_burn_conditional_asset_basic() {
    ...
    let amount = 1000;
    let coin = coin_escrow::mint_conditional_asset<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, amount, ctx
    );
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == amount, 0);
...
fun test_burn_conditional_stable_basic() {
    ...
    assert!(coin_escrow::get_stable_supply<TEST_COIN_A, TEST_COIN_B, COND_0_STABLE>(&escrow, 0) == amount, 0);
    ...
    // Verify supply reduced to zero
    assert!(coin_escrow::get_stable_supply<TEST_COIN_A, TEST_COIN_B, COND_0_STABLE>(&escrow, 0) == 0, 1);
...
fun test_burn_partial_supply() {
    ...
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 500, 1);
    ...
    // Supply should be 0
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 0, 2);
...
fun test_burn_different_outcomes_independently() {
    ...
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 0, 0);
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_1_ASSET>(&escrow, 1) == 2000, 1);
    ...
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 0, 2);
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_1_ASSET>(&escrow, 1) == 0, 3);
...
fun test_burn_asset_and_stable_independently() {
    ...
    // Verify both supplies are 0
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 0, 2);
    assert!(coin_escrow::get_stable_supply<TEST_COIN_A, TEST_COIN_B, COND_0_STABLE>(&escrow, 0) == 0, 3);
...
fun test_burn_conditional_asset_out_of_bounds() {
    ...
    coin_escrow::burn_conditional_asset<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 1, coin // Wrong outcome index
    );
...
fun test_mint_burn_cycle_maintains_zero_supply() {
    ...
    assert!(coin_escrow::get_asset_supply<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&escrow, 0) == 0, 3);
...
fun test_deposit_zero_asset_nonzero_stable() {
    ...
    let (asset_amt, stable_amt) = coin_escrow::deposit_spot_coins(&mut escrow, asset_coin, stable_coin);
...
fun test_deposit_asset_and_mint_basic() {
    ...
    let cond_asset = coin_escrow::deposit_asset_and_mint_conditional<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow,
        0, // outcome_index
        spot_asset,
        ctx
...
fun test_deposit_stable_and_mint_basic() {
    ...
    // Deposit and mint in one operation
    let cond_stable = coin_escrow::deposit_stable_and_mint_conditional<TEST_COIN_A, TEST_COIN_B, COND_0_STABLE>(
        &mut escrow,
        0, // outcome_index
        spot_stable,
        ctx
...
fun test_deposit_mint_for_different_outcomes() {
    ...
    let cond_0 = coin_escrow::deposit_asset_and_mint_conditional<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, spot_0, ctx
    );

    // Deposit and mint for outcome 1
    let spot_1 = coin::mint_for_testing<TEST_COIN_A>(2000, ctx);
    let cond_1 = coin_escrow::deposit_asset_and_mint_conditional<TEST_COIN_A, TEST_COIN_B, COND_1_ASSET>(
        &mut escrow, 1, spot_1, ctx
    );
...
fun test_burn_asset_and_withdraw_basic() {
    ...
    let withdrawn_spot = coin_escrow::burn_conditional_asset_and_withdraw<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow,
        0, // outcome_index
...
fun test_burn_stable_and_withdraw_basic() {
    ...
    let withdrawn_spot = coin_escrow::burn_conditional_stable_and_withdraw<TEST_COIN_A, TEST_COIN_B, COND_0_STABLE>(
        &mut escrow,
        0, // outcome_index
        1000, // amount
        ctx
...
fun test_full_cycle_deposit_mint_burn_withdraw() {
    ...
    let withdrawn = coin_escrow::burn_conditional_asset_and_withdraw<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, 500, ctx
    );
...
fun test_multiple_deposit_mint_burn_withdraw_cycles() {
    ...
    let cond1 = coin_escrow::deposit_asset_and_mint_conditional<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, spot1, ctx
    );
    coin_escrow::burn_conditional_asset<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&mut escrow, 0, cond1);
    let withdrawn1 = coin_escrow::burn_conditional_asset_and_withdraw<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, 500, ctx
    );
    ...
    let cond2 = coin_escrow::deposit_asset_and_mint_conditional<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, spot2, ctx
    );
    coin_escrow::burn_conditional_asset<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&mut escrow, 0, cond2);
    let withdrawn2 = coin_escrow::burn_conditional_asset_and_withdraw<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, 1000, ctx
    );
    ...
    let cond3 = coin_escrow::deposit_asset_and_mint_conditional<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, spot3, ctx
    );
    coin_escrow::burn_conditional_asset<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&mut escrow, 0, cond3);
    let withdrawn3 = coin_escrow::burn_conditional_asset_and_withdraw<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
...
fun test_cross_outcome_operations() {
    ...
    coin_escrow::burn_conditional_asset<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(&mut escrow, 0, cond_0);
    ...
    let withdrawn = coin_escrow::burn_conditional_asset_and_withdraw<TEST_COIN_A, TEST_COIN_B, COND_1_ASSET>(
...
fun test_deposit_mint_zero_amount_edge_case() {
    ...
    let cond_zero = coin_escrow::deposit_asset_and_mint_conditional<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, spot_zero, ctx
    );
...
fun test_deposit_mint_large_amounts() {
    ...
    let cond_large = coin_escrow::deposit_asset_and_mint_conditional<TEST_COIN_A, TEST_COIN_B, COND_0_ASSET>(
        &mut escrow, 0, spot_large, ctx
    );
...
fun test_split_stable_complete_set_2_basic() {
    ...
    // Verify supplies
    let supply_0 = coin_escrow::get_stable_supply<TEST_COIN_A, TEST_COIN_B, COND_0_STABLE>(&escrow, 0);
    let supply_1 = coin_escrow::get_stable_supply<TEST_COIN_A, TEST_COIN_B, COND_1_STABLE>(&escrow, 1);
