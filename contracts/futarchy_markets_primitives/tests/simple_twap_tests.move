...
fun test_initialization_default() {
    ...
    let oracle = simple_twap::new_default(initial_price, &clock);

    // Verify initialization
    assert!(simple_twap::last_price(&oracle) == initial_price, 0);
    assert!(simple_twap::get_twap(&oracle) == initial_price, 1);
    assert!(simple_twap::window_size_ms(&oracle) == ONE_MINUTE_MS, 2);
    assert!(simple_twap::max_movement_ppm(&oracle) == 10_000, 3); // 1%
...
fun test_custom_config() {
    ...
    assert!(simple_twap::window_size_ms(&oracle) == 120_000, 0);
    assert!(simple_twap::max_movement_ppm(&oracle) == 20_000, 1);
...
fun test_single_window_no_cap() {
    ...
    let mut oracle = simple_twap::new_default(initial_price, &clock);
    ...
    assert!(simple_twap::get_twap(&oracle) == initial_price, 0);
...
fun test_single_window_with_cap_upward() {
    ...
    assert!(simple_twap::get_twap(&oracle) == expected, 0);
...
fun test_single_window_small_move_no_cap() {
    ...
    assert!(simple_twap::get_twap(&oracle) == new_price, 0);
...
fun test_multi_window_gradual_approach() {
    ...
    let mut oracle = simple_twap::new_default(price, &clock);
    ...
    price = 105 * SCALE;
    clock::set_for_testing(&mut clock, ONE_MINUTE_MS);
    simple_twap::update(&mut oracle, price, &clock);
    // TWAP: $100 → $101 (capped at 1%)
    assert!(simple_twap::get_twap(&oracle) == 101 * SCALE, 0);
    ...
    assert!(simple_twap::get_twap(&oracle) == expected2, 1);
    ...
    assert!(simple_twap::get_twap(&oracle) == expected3, 2);
...
fun test_dynamic_cap_grows_with_twap() {
    ...
    clock::set_for_testing(&mut clock, ONE_MINUTE_MS);
    simple_twap::update(&mut oracle, 200 * SCALE, &clock);
    assert!(simple_twap::get_twap(&oracle) == 101 * SCALE, 0);

    // Second window: $101 → $102.01 (cap = $1.01, growing!)
    clock::set_for_testing(&mut clock, 2 * ONE_MINUTE_MS);
    simple_twap::update(&mut oracle, 200 * SCALE, &clock);
    assert!(simple_twap::get_twap(&oracle) == 102_010_000_000_000, 1);

    // Third window: $102.01 → $103.0301 (cap = $1.0201)
    clock::set_for_testing(&mut clock, 3 * ONE_MINUTE_MS);
    simple_twap::update(&mut oracle, 200 * SCALE, &clock);
    assert!(simple_twap::get_twap(&oracle) == 103_030_100_000_000, 2);
...
fun test_incomplete_window() {
    ...
    assert!(simple_twap::get_cumulative_price(&oracle) > 0, 1);
...
fun test_price_volatility_averaging() {
    ...
    simple_twap::update(&mut oracle, 50 * SCALE, &clock);

    // Next 40 seconds at $150 (window completes)
    clock::set_for_testing(&mut clock, ONE_MINUTE_MS);
    simple_twap::update(&mut oracle, 150 * SCALE, &clock);
...
fun test_realistic_oracle_usage() {
    ...
    let final_twap = simple_twap::get_twap(&oracle);
