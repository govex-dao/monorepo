...
fun test_multi_coin_treasury_actions() {
    ...
    scenario.next_tx(admin);
    {
        ...
        // Execute TESTCOIN transfer
        treasury_actions::execute_outcome_actions<TESTCOIN>(
            &mut registry,
            &mut treasury,
            &dao,
            &clock,
    ...
    // Verify Bob received both coins
    scenario.next_tx(bob);
    {
        ...
        let testcoin = scenario.take_from_sender<Coin<TESTCOIN>>();
        assert!(testcoin.value() == 10_000_000_000, 2);
        scenario.return_to_sender(testcoin);
    };
    ...
    scenario.next_tx(admin);
    {
        ...
        assert!(testcoin_balance == 40_000_000_000, 4); // 50 - 10 = 40
