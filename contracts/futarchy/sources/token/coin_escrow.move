module futarchy::coin_escrow {
    ...
    fun assert_supplies_initialized<AssetType, StableType>(escrow: &TokenEscrow<AssetType, StableType>) {
        let outcome_count = market_state::outcome_count(&escrow.market_state);
        assert!(vector::length(&escrow.outcome_asset_supplies) == outcome_count &&
    ...
    public fun get_balances<AssetType, StableType>(
        ...
        (
            balance::value(&escrow.escrowed_asset),
            balance::value(&escrow.escrowed_stable)
        )
    }
