module futarchy::advance_stage {
    ...
    public fun try_advance_state<AssetType, StableType>(
        ...
        if (proposal::state(proposal) == STATE_REVIEW && elapsed >= proposal::get_review_period_ms(proposal)) {
    ...
    public fun finalize<AssetType, StableType>(
        ...
        while (i < pools_count) {
            ...
            if (i == 0) {
                ...
                let threshold_price = (base_twap * (TWAP_BASIS_POINTS + (proposal::get_twap_threshold(proposal) as u128))) / TWAP_BASIS_POINTS;
