module futarchy::fee {
    ...
    public fun deposit_stable_fees<StableType>(
        ...
        if (dynamic_field::exists_with_type<StableFeeRegistry<StableType>, StableCoinBalance<StableType>>(&fee_manager.id, StableFeeRegistry<StableType> {})) {
            let fee_balance_wrapper = dynamic_field::borrow_mut<StableFeeRegistry<StableType>, StableCoinBalance<StableType>>(&mut fee_manager.id, StableFeeRegistry<StableType> {});
    ...
    public entry fun withdraw_stable_fees<StableType>(
        ...
        assert!(
            dynamic_field::exists_with_type<StableFeeRegistry<StableType>, StableCoinBalance<StableType>>(
    ...
    public fun get_stable_fee_balance<StableType>(fee_manager: &FeeManager): u64 {
        if (dynamic_field::exists_with_type<StableFeeRegistry<StableType>, StableCoinBalance<StableType>>(&fee_manager.id, StableFeeRegistry<StableType> {})) {
            let balance_wrapper = dynamic_field::borrow<StableFeeRegistry<StableType>, StableCoinBalance<StableType>>(&fee_manager.id, StableFeeRegistry<StableType> {});
