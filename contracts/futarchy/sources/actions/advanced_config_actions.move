...
module futarchy::advanced_config_actions;
...
const EInvalidSlashDistribution: u64 = 5;
...
public fun do_update_metadata_table<Outcome: store, IW: drop>(
    ...
    // 1. Add/update entries for each key-value pair in action.keys/values
    // 2. Remove entries for each key in action.keys_to_remove
    let _ = config;
    let _ = action;
...
public fun do_update_twap_params<Outcome: store, IW: drop>(
    ...
    do_update_twap_config<Outcome, IW>(executable, account, version, iw, ctx);
}
...
public fun do_update_fee_params<Outcome: store, IW: drop>(
    ...
    do_update_queue_params<Outcome, IW>(executable, account, version, iw, ctx);
}