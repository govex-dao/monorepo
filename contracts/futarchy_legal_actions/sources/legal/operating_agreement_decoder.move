...
module futarchy_legal_actions::operating_agreement_decoder;
...
use futarchy_legal_actions::operating_agreement::{
...
public fun decode_operating_agreement_action(
    ...
    if (line_id.is_some()) {
        let id = line_id.destroy_some();
        fields.push_back(schema::new_field(
            b"line_id".to_string(),
            object::id_to_bytes(&id).to_string(),
            b"ID".to_string(),
        ));
    } else {
