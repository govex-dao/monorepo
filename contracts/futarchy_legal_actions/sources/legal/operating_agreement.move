...
module futarchy_legal_actions::operating_agreement;
...
const ELineNotYetImmutable: u64 = 21;
...
public struct AgreementLine has store, drop {
    ...
    /// Timestamp when line becomes immutable (milliseconds)
    /// Until this time, the line can be changed. After this time, it's locked forever.
...
/// Create a new InsertLineAtBeginningAction
...
/// Create a new BatchOperatingAgreementAction
...
public fun new_operating_agreement_action(
    ...
    OperatingAgreementAction { action_type, line_id, text, difficulty }
}
...
/// Get action type from OperatingAgreementAction
public fun get_action_type(action: &OperatingAgreementAction): u8 {
    action.action_type
}

/// Get line_id option from OperatingAgreementAction
public fun get_line_id(action: &OperatingAgreementAction): &Option<ID> {
    &action.line_id
}

/// Get text option from OperatingAgreementAction
public fun get_text(action: &OperatingAgreementAction): &Option<String> {
    &action.text
}

/// Get difficulty option from OperatingAgreementAction
public fun get_difficulty(action: &OperatingAgreementAction): &Option<u64> {
    &action.difficulty
}

/// Get actions from BatchOperatingAgreementAction
public fun get_batch_actions(batch: &BatchOperatingAgreementAction): &vector<OperatingAgreementAction> {
    &batch.actions
}
...
public struct AgreementReadWithStatus has copy, drop {
    ...
    immutable_from: vector<Option<u64>>, // Scheduled immutability times
    allow_insert: bool,
    allow_remove: bool,
    global_immutable: bool,
    timestamp_ms: u64,
...
public fun new(
    ...
    while (i < initial_lines.length()) {
        ...
        let line = AgreementLine {
            ...
            immutable_from: option::none(),  // No scheduled immutability by default
        };
...
// NOTE: The execute_* functions below are commented out as they use the old executable.next_action() pattern
// which is no longer part of the current architecture. The operating_agreement_actions module now handles
// all action execution using the new pattern with ActionSpec and BCS deserialization.
...
fun update_line_internal(
    ...
    // Check if line is immutable (either permanently or by scheduled time)
    assert!(!is_line_immutable_now(line, clock.timestamp_ms()), ELineIsImmutable);
...
fun remove_line_internal(
    ...
    // Check if the line is immutable (either permanently or by scheduled time) before removing
...
/// Set a line as immutable immediately (one-way: can only go from mutable to immutable)
/// Note: If line has scheduled immutability (immutable_from), you cannot make it immutable
/// before that scheduled time. This prevents bypassing the scheduled lock period.
public fun set_line_immutable(
    ...
    assert!(!is_line_immutable_now(line, now), EAlreadyImmutable);

    // If line has scheduled immutability that hasn't been reached yet,
    // cannot make it immutable early (this would bypass the lock period)
    if (line.immutable_from.is_some()) {
        assert!(now >= *line.immutable_from.borrow(), ECannotMakeImmutableBeforeScheduled);
    };
...
public fun get_line_difficulty(agreement: &OperatingAgreement, line_id: ID): u64 {
    assert!(df::exists_<LineKey>(&agreement.id, LineKey { id: line_id }), ELineNotFound);
...
/// Check if a specific line is permanently immutable (immutable = true)
/// Note: This only checks the permanent flag, not scheduled immutability
/// Use is_line_immutable_at_time() to check time-based immutability
...
/// Check if a specific line is currently immutable (considering both permanent and time-based)
/// This is the recommended function to use for runtime immutability checks
public fun is_line_immutable_at_time(
    agreement: &OperatingAgreement,
    line_id: ID,
    current_time_ms: u64
): bool {
    assert!(df::exists_<LineKey>(&agreement.id, LineKey { id: line_id }), ELineNotFound);
    let line = df::borrow<LineKey, AgreementLine>(&agreement.id, LineKey { id: line_id });
    is_line_immutable_now(line, current_time_ms)
}
...
/// Check if a line is currently immutable based on time
/// Returns true if:
/// - line.immutable is true (permanently immutable), OR
/// - line.immutable_from is set and current time >= immutable_from (scheduled immutability reached)
public fun is_line_immutable_now(line: &AgreementLine, current_time_ms: u64): bool {
    ...
    if (line.immutable_from.is_some()) {
        if (current_time_ms >= *line.immutable_from.borrow()) {
            return true
        }
    };
...
/// Insert a line with scheduled immutability (becomes immutable at future time)
/// The line starts mutable but automatically becomes immutable when immutable_from is reached
/// This is useful for:
/// - Cliff periods: "Founder allocation locked until cliff date"
/// - Gradual finalization: "Constitution becomes immutable 1 year after adoption"
public fun insert_line_with_scheduled_immutability_after(
    agreement: &mut OperatingAgreement,
    prev_line_id: ID,
    text: String,
    difficulty: u64,
    ...
    // Validate immutable_from time is in the future but not too far
    let now = clock.timestamp_ms();
    ...
    let new_line = AgreementLine {
        text,
        difficulty,
        immutable: false,  // Starts mutable, becomes immutable at immutable_from
        prev: option::some(prev_line_id),
        next: prev_line_next,
        line_type: LINE_TYPE_PERMANENT,
        expires_at: option::none(),
        effective_from: option::none(),
        immutable_from: option::some(immutable_from_ms),  // Scheduled immutability
    };
...
public fun emit_current_state_event_with_status(agreement: &OperatingAgreement, clock: &Clock) {
    ...
    let mut immutable_from_vec = vector[];
    ...
    while (i < ordered_ids.length()) {
        ...
        immutable_from_vec.push_back(line.immutable_from);
        i = i + 1;
    };
    ...
    event::emit(AgreementReadWithStatus {
        ...
        immutable_from: immutable_from_vec,
        allow_insert: agreement.allow_insert,
        allow_remove: agreement.allow_remove,
        global_immutable: agreement.immutable,
        timestamp_ms: now,
...
/// Initialize operating agreement during DAO creation
/// Called directly by PTB during init phase
public entry fun init_create_operating_agreement(
    account: &mut Account<FutarchyConfig>,
    lines: vector<vector<u8>>,      // Agreement lines as bytes
    difficulties: vector<u64>,       // Difficulty for each line (0-10000 basis points)
    immutable_indices: vector<u64>,  // Which lines should be immutable
    allow_insert: bool,              // Whether new lines can be added
    allow_remove: bool,              // Whether lines can be removed
    ctx: &mut TxContext,
) {
    // Validate inputs
    assert!(vector::length(&lines) == vector::length(&difficulties), EIncorrectLengths);
    assert!(vector::length(&lines) <= MAX_LINES_PER_AGREEMENT, ETooManyLines);

    // Convert bytes to strings
    let mut string_lines = vector::empty<String>();
    let mut i = 0;
    while (i < vector::length(&lines)) {
        vector::push_back(
            &mut string_lines,
            std::string::utf8(*vector::borrow(&lines, i))
        );
        i = i + 1;
    };

    // Create the operating agreement
    let mut agreement = new(
    ...
    // Set immutable lines
    let mut j = 0;
    while (j < vector::length(&immutable_indices)) {
        let _index = *vector::borrow(&immutable_indices, j);
        // Mark lines as immutable by index
        // This would need access to the internal structure
        j = j + 1;
    };

    // Set insert/remove permissions
    agreement.allow_insert = allow_insert;
    agreement.allow_remove = allow_remove;

    // Store in Account's dynamic fields
