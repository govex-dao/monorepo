...
module futarchy_payments::payment_decoder_tests {
    ...
    fun test_deserialize_create_payment_action_with_none_interval() {
        let action = payment_actions::new_create_payment_action<SUI>(
            ...
            10u64, true, string::utf8(b"Test"), 100u64, 1000u64, 5u64
        );
    ...
    fun test_deserialize_create_payment_action_zero_values() {
        let action = payment_actions::new_create_payment_action<SUI>(
            0u8, 0u8, @0x0, 0u64, 0u64, 0u64,
            option::none(), 0u64, false, string::utf8(b""), 0u64, 0u64, 0u64
        );
    ...
    fun test_decode_create_payment_action() {
        ...
        ts::next_tx(&mut scenario, ADMIN);
        {
            let action = payment_actions::new_create_payment_action<SUI>(
                1u8, 2u8, ALICE, 1000u64, 100u64, 200u64,
                option::some(50u64), 10u64, true,
                string::utf8(b"Test payment"), 100u64, 1000u64, 5u64
            );

            let action_data = bcs::to_bytes(&action);

            // Get decoder and decode
            let decoder = schema::borrow_decoder<payment_decoder::CreatePaymentActionDecoder>(&registry);
    ...
    fun test_decode_cancel_payment_action() {
        ...
        ts::next_tx(&mut scenario, ADMIN);
        {
            ...
            let decoder = schema::borrow_decoder<payment_decoder::CancelPaymentActionDecoder>(&registry);
    ...
    fun test_register_decoders() {
        ...
        ts::next_tx(&mut scenario, ADMIN);
        {
            ...
            let _create_decoder = schema::borrow_decoder<payment_decoder::CreatePaymentActionDecoder>(&registry);

            // Verify CancelPaymentActionDecoder is registered
            let _cancel_decoder = schema::borrow_decoder<payment_decoder::CancelPaymentActionDecoder>(&registry);
        };
    ...
    fun test_roundtrip_create_payment_action() {
        ...
        ts::next_tx(&mut scenario, ADMIN);
        {
            ...
            let original = payment_actions::new_create_payment_action<SUI>(
                5u8, 7u8, ALICE, 5000u64, 500u64, 1500u64,
                option::some(100u64), 20u64, false,
                string::utf8(b"Roundtrip test"), 200u64, 5000u64, 10u64
            );
