...
use futarchy::oracle::{Self, Oracle, ETimestampRegression, EInternalTwapError};
use std::u128;
use std::u256;
use std::u64;
...
fun test_accumulate_pass_internal_error_final_timestamp_mismatch_check_complex_path() {
    ...
    let final_timestamp =
        MARKET_START_TIME + 10_000 + (WINDOW_SIZE - 10_000) + 2 * WINDOW_SIZE + 20_000;
...
fun test_accumulate_stage1_only_intra_window_no_boundary_cross() {
    ...
    assert!(
        oracle::get_total_cumulative_price_for_testing(&oracle_inst) == expected_total_cum_price,
        2,
    );
...
fun test_accumulate_stage1_only_completes_window_exactly() {
    ...
    let current_lwe_cum_price = oracle::get_last_window_end_cumulative_price_for_testing(
        &oracle_inst,
    );
...
#[test]
fun test_accumulate_one_full_window_from_boundary_via_stage1() {
    // Renamed from "stage2_only_single_full_window_from_boundary"
    let mut scenario = setup_scenario();
    let test_ctx = ctx(&mut scenario);
    let mut oracle_inst = default_oracle(test_ctx); // last_ts=0, last_window_end=0, last_window_twap=INIT_PRICE
    ...
    assert!(
        oracle::get_last_window_end_cumulative_price_for_testing(&oracle_inst) == price_contribution,
        5,
    );
...
#[test]
fun test_accumulate_stage1_then_stage2_multiple_full_windows_from_boundary() {
    // Renamed from "stage2_only_multiple_full_windows_from_boundary"
    let mut scenario = setup_scenario();
    let test_ctx = ctx(&mut scenario);
    let mut oracle_inst = default_oracle(test_ctx);
    ...
    let expected_total_cum_price =
        (price_for_multi_window as u256) * (num_total_windows as u256) * (WINDOW_SIZE as u256);
    assert!(
        oracle::get_total_cumulative_price_for_testing(&oracle_inst) == expected_total_cum_price,
        4,
    );
    assert!(
        oracle::get_last_window_end_cumulative_price_for_testing(&oracle_inst) == expected_total_cum_price,
        5,
    );
...
#[test]
fun test_accumulate_stage1_full_stage2_full_then_stage3_partial_from_boundary() {
    // Adjusted name
    let mut scenario = setup_scenario();
    let test_ctx = ctx(&mut scenario);
...
fun test_accumulate_very_long_duration_many_full_windows() {
    ...
    let expected_total_cum_price =
        (price as u256) * (num_total_windows as u256) * (WINDOW_SIZE as u256);
    assert!(
        oracle::get_total_cumulative_price_for_testing(&oracle_inst) == expected_total_cum_price,
        4,
    );
