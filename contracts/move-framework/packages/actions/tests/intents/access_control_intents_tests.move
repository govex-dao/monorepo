...
fun test_execute_borrow_and_return_separately() {
    ...
    let outcome = Outcome {};
    let params = intents::new_params(key, b"Test".to_string(), vector[0], 1000, &clock, scenario.ctx());

    let auth2 = account.new_auth(version::current(), Witness());
    access_control_intents::request_borrow_cap<_, _, TestCap>(auth2, &mut account, params, outcome, scenario.ctx());
    ...
    // Execute borrow
    let cap = access_control_intents::execute_borrow_cap<Config, Outcome, TestCap>(&mut executable, &mut account);

    // Do something with the cap
    assert!(cap.value == 100, 0);

    // Execute return
    access_control_intents::execute_return_cap<Config, Outcome, TestCap>(&mut executable, &mut account, cap);
...
fun test_multiple_borrow_return_cycles() {
    ...
    let outcome = Outcome {};
    let params1 = intents::new_params(key1, b"Cycle 1".to_string(), vector[0], 1000, &clock, scenario.ctx());
    let auth1 = account.new_auth(version::current(), Witness());
    access_control_intents::request_borrow_cap<_, _, TestCap>(auth1, &mut account, params1, outcome, scenario.ctx());

    // Execute first cycle
    let (_, mut exec1) = account.create_executable<_, Outcome, _>(key1, &clock, version::current(), Witness(), scenario.ctx());
    let cap1 = access_control_intents::execute_borrow_cap<Config, Outcome, TestCap>(&mut exec1, &mut account);
    assert!(cap1.value == 999, 0);
    access_control_intents::execute_return_cap<Config, Outcome, TestCap>(&mut exec1, &mut account, cap1);
    account.confirm_execution(exec1);

    // Create second intent
    let key2 = b"cycle2".to_string();
    let params2 = intents::new_params(key2, b"Cycle 2".to_string(), vector[0], 2000, &clock, scenario.ctx());
    let auth2 = account.new_auth(version::current(), Witness());
    access_control_intents::request_borrow_cap<_, _, TestCap>(auth2, &mut account, params2, outcome, scenario.ctx());
    ...
    let cap2 = access_control_intents::execute_borrow_cap<Config, Outcome, TestCap>(&mut exec2, &mut account);
    assert!(cap2.value == 999, 1);
    access_control_intents::execute_return_cap<Config, Outcome, TestCap>(&mut exec2, &mut account, cap2);
