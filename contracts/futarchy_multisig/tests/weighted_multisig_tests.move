...
/// Tests core voting, time locks, dead man switch, and stale proposal detection
#[test_only]
module futarchy_multisig::weighted_multisig_tests {
    ...
    fun test_new_creates_valid_multisig() {
        ...
        assert!(weighted_multisig::last_activity_ms(&multisig) > 0);
    ...
    #[test]
    fun test_set_dead_man_switch_recipient() {
        let clock = setup_clock();
        ...
        // Set recipient as the DAO itself (recipient_dao_id = None)
        weighted_multisig::set_dead_man_switch_recipient(
            &mut multisig,
            dao_id,
            option::none()
        );

        assert!(weighted_multisig::has_dead_man_switch(&multisig));
        assert_eq(*weighted_multisig::dead_man_switch_recipient(&multisig).borrow(), dao_id);

        clock::destroy_for_testing(clock);
    }

    #[test]
    fun test_set_dead_man_switch_timeout() {
        let clock = setup_clock();
        ...
        weighted_multisig::set_dead_man_switch_timeout(&mut multisig, THIRTY_DAYS_MS);

        assert_eq(weighted_multisig::dead_man_switch_timeout_ms(&multisig), THIRTY_DAYS_MS);

        clock::destroy_for_testing(clock);
    }

    #[test]
    #[expected_failure(abort_code = weighted_multisig::EInvalidDeadManSwitchRecipient)]
    fun test_set_dead_man_switch_recipient_fails_if_wrong_dao() {
        let clock = setup_clock();
        ...
        // Try to set recipient with wrong DAO ID - should fail!
        weighted_multisig::set_dead_man_switch_recipient(
            &mut multisig,
            object::id_from_address(@0xBAD),
            option::some(wrong_dao_id)
        );
    ...
    #[test]
    fun test_is_inactive_returns_false_when_no_timeout() {
        let mut clock = setup_clock();
        clock::set_for_testing(&mut clock, 1000000);
        ...
        let dao_id = object::id_from_address(@0xDA0);
        weighted_multisig::set_dao_id(&mut multisig, dao_id);
        weighted_multisig::set_dead_man_switch_recipient(&mut multisig, dao_id, option::none());

        // No timeout set (0 = disabled)
        weighted_multisig::set_dead_man_switch_timeout(&mut multisig, 0);

        // Advance time way into the future
        clock::increment_for_testing(&mut clock, THIRTY_DAYS_MS * 12);

        // Should still be active (dead man switch disabled)
        assert!(!weighted_multisig::is_inactive(&multisig, &clock));

        clock::destroy_for_testing(clock);
    }

    #[test]
    fun test_is_inactive_returns_false_before_timeout() {
        let mut clock = setup_clock();
        clock::set_for_testing(&mut clock, 1000000);
        ...
        clock::increment_for_testing(&mut clock, THIRTY_DAYS_MS - 1000);
    ...
    #[test]
    fun test_is_inactive_returns_true_after_timeout() {
        let mut clock = setup_clock();
        clock::set_for_testing(&mut clock, 1000000);
    ...
    #[test]
    fun test_bump_last_activity_resets_inactivity() {
        let mut clock = setup_clock();
        clock::set_for_testing(&mut clock, 1000000);
        ...
        // Bump activity - resets timer
        weighted_multisig::bump_last_activity(&mut multisig, &clock);
        ...
        // Still active (activity was bumped)
        assert!(!weighted_multisig::is_inactive(&multisig, &clock));
    ...
    #[test]
    fun test_dead_man_switch_full_scenario() {
        let mut clock = setup_clock();
        clock::set_for_testing(&mut clock, 1000000);

        // Create main council
        let mut main_council = weighted_multisig::new(
            vector[ALICE, BOB],
            vector[50, 50],
            75,
            &clock
        ...
        // Create recipient council with same DAO
        let dao_id = object::id_from_address(@0xDA0);
        let mut recipient_council = weighted_multisig::new(
            vector[CAROL, DAVE, EVE],
            vector[33, 33, 34],
            67,
            &clock
        ...
        // Set up DAO relationship
        weighted_multisig::set_dao_id(&mut main_council, dao_id);
        weighted_multisig::set_dao_id(&mut recipient_council, dao_id);

        // Configure dead man switch
        let recipient_id = object::id_from_address(@0x123);
        weighted_multisig::set_dead_man_switch_recipient(
            &mut main_council,
            recipient_id,
            option::some(dao_id) // Recipient is another multisig in same DAO
        );
        weighted_multisig::set_dead_man_switch_timeout(&mut main_council, THIRTY_DAYS_MS);

        // Initially active
        assert!(!weighted_multisig::is_inactive(&main_council, &clock));
        assert!(!weighted_multisig::can_trigger_dead_man_switch(&main_council, &recipient_council, &clock));
        ...
        // Validate recipient at execution time
        weighted_multisig::validate_recipient_at_execution(&main_council, &recipient_council);
