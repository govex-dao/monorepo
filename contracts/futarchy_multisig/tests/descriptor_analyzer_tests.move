...
fun test_mode_2_needs_both() {
    ...
    let requirement = create_test_requirement(
        true,
        true,
        option::some(object::id_from_address(@0x1)),
        2,
    );
    assert_eq(descriptor_analyzer::needs_dao(&requirement), true);
...
fun test_all_mode_combinations() {
    ...
    let approval_combos = vector[(false, false), (false, true), (true, false), (true, true)];
    ...
    while (i < modes.length()) {
        ...
        let requirement = create_test_requirement(
            needs_dao,
            needs_council,
            if (needs_council) { option::some(object::id_from_address(@0x1)) } else {
                option::none()
            },
            mode,
        );
...
fun test_different_council_ids() {
    ...
    assert!(
        *descriptor_analyzer::council_id(&req1).borrow() != *descriptor_analyzer::council_id(&req2).borrow(),
    );
}
