...
fun test_pause_grant() {
    ...
    ts::next_tx(&mut scenario, ADMIN);
    {
        let mut grant = ts::take_shared_by_id<PriceBasedMintGrant<ASSET, STABLE>>(&scenario, grant_id);
...
fun test_pause_already_paused_fails() {
    ...
    ts::next_tx(&mut scenario, ADMIN);
    let grant_id = oracle_actions::create_employee_option<ASSET, STABLE>(
        RECIPIENT, 100_000, 1_000_000, 0, 1, 3_000_000_000, 10,
        object::id_from_address(@0xDA0), &clock, ts::ctx(&mut scenario),
    );
